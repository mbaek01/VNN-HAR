...
class VNDGCNNEncoder(nn.Module):
    def __init__(self, num_neighbors, dims=[1, 21, 21, 42, 85, 341], use_bn=False):
        ...
        for dim_in, dim_out in zip(dims[:-2], dims[1:-1]):
            layers += [VNLinearLeakyReLU(2 * dim_in, dim_out, batch_norm=use_bn, dim=5)]

        layers += [VNLinearLeakyReLU(sum(dims[1:-1]), dims[-1], batch_norm=use_bn, dim=4, share_nonlinearity=True)]
