...
class VN_Inv_Baseline_Attn(nn.Module):
    def __init__(self, input, nb_classes, nb_units):
        ...
        self.VNInvEncoderLayer1 = VNInvEncoderLayer(d_model = self.nb_units, n_heads=4 , d_ff = self.nb_units*4)
        self.VNInvEncoderLayer2 = VNInvEncoderLayer(d_model = self.nb_units, n_heads=4 , d_ff = self.nb_units*4)
    ...
    def forward(self, x):                                   # (B, 1, L, C)
        ...
        # reshape C -> 3, C//3
        x = vn_c_reshape(x, self.time_length).squeeze(1)    # (B, L, 3, C//3)

        # Initial fc layer
        x = self.vn_act(self.fc1(x))                        # (B, L, 3, D)

        # Self-Attention Encoder
        x = self.VNInvEncoderLayer1(x)                      # (B, L, 3, D)
        x = self.VNInvEncoderLayer2(x)                      # (B, L, 3, D)

        # Global Temporal Attention
        x = self.VNInvAttentionWithContext(x)               # (B, 3, D)
...
class VN_Baseline_Attn(nn.Module):
    ...
    def forward(self, x):
        ...
        # Shape: (B, 1, L, 3, C); C = nb_units // 3

        x = self.vn_act(self.fc1(x)).squeeze(1)
