...
class Rotate:
    def __init__(self, n: int, method: str, device: Optional[Device], dtype=Optional[torch.dtype], angle=None):
        """
        Args:
        ...
        if not angle:
            self.angle = torch.rand(self.n, device=device, dtype=self.dtype) * (360.0 / 180.0 * math.pi)
        else:
            self.angle = torch.full((self.n,), angle / 180.0 * math.pi, device=device, dtype=self.dtype) 

        if self.method == "so3":
            self.rotation = self.quaternion_to_matrix(self._random_quaternions(dtype=self.dtype))
        elif self.method in ["x", "y", "z"]:
            self.rotation = self._axis_angle_rotation(self.angle)
        else:
    ...
    def _axis_angle_rotation(self, angle: torch.Tensor) -> torch.Tensor:
        """
